            ...
            <div key={entry.id} className={`card p-4 space-y-2 ${isActive ? '' : 'opacity-60'}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  {isActive ? (
                    <button onClick={() => toggleCustom('customHabits', entry.id)} className="text-xs text-slate-400 hover:text-red-500">
                      {locale === 'en' ? 'Stop' : 'Arrêter'}
                    </button>
                  ) : (
                    <>
                      <button onClick={() => toggleCustom('customHabits', entry.id)} className="text-xs text-slate-400 hover:text-brand-600">
                        {locale === 'en' ? 'Resume' : 'Reprendre'}
                      </button>
                      <button onClick={() => removeCustom('customHabits', entry.id)} className="text-xs text-slate-400 hover:text-red-500">
